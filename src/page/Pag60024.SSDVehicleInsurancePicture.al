page 60024 "SSD Vehicle Insurance Picture"
{
    Caption = 'Vehicle Insurance Picture';
    DeleteAllowed = false;
    InsertAllowed = false;
    LinksAllowed = false;
    PageType = CardPart;
    ...
    begin
        if Rec.Insurance.Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec.Insurance);
            Rec.Insurance.ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
    ...
    begin
        DeleteExportEnabled:=Rec.Insurance.Count <> 0;
    end;
