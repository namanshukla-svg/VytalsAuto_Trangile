page 60021 "SSD Vehicle Chassis Picture"
{
    ...
    layout
    {
        area(Content)
        {
            field(Picture; Rec."Chassis Number")
            {
                ApplicationArea = All;
                ShowCaption = false;
    ...
    begin
        if Rec."Chassis Number".Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec."Chassis Number");
            Rec."Chassis Number".ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
    ...
    begin
        DeleteExportEnabled:=Rec."Chassis Number".Count <> 0;
    end;
