page 60019 "SSD Vehicle RC Picture"
{
    Caption = 'RC Picture';
    DeleteAllowed = false;
    InsertAllowed = false;
    ...
    begin
        if Rec."Vehicle RC".Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec."Vehicle RC");
            Rec."Vehicle RC".ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
    ...
    begin
        DeleteExportEnabled:=Rec."Vehicle RC".Count <> 0;
    end;
