page 60023 "SSD Vehicle Pollution Picture"
{
    ...
    layout
    {
        area(Content)
        {
            field(Picture; Rec."Pollution Certificate")
            {
                ApplicationArea = All;
                ShowCaption = false;
    ...
    begin
        if Rec."Pollution Certificate".Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec."Pollution Certificate");
            Rec."Pollution Certificate".ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
