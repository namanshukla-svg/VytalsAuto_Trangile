page 60020 "SSD Vehicle No. Plate Picture"
{
    Caption = 'Vehicle No. Plate Picture';
    DeleteAllowed = false;
    InsertAllowed = false;
    LinksAllowed = false;
    PageType = CardPart;
    ...
    layout
    {
        area(Content)
        {
            field(Picture; Rec."No. Plate")
            {
                ApplicationArea = All;
                ShowCaption = false;
                ToolTip = 'Specifies the picture that has been inserted for the vehicle.';
            }
        }
    }
    actions
    ...
    begin
        if Rec."No. Plate".Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec."No. Plate");
            Rec."No. Plate".ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
    ...
    begin
        DeleteExportEnabled:=Rec."No. Plate".Count <> 0;
    end;
