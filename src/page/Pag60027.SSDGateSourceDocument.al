page 60027 "SSD Gate Source Document"
{
    ...
    begin
        if Rec."SSD Source Document".Count() > 0 then if not Confirm(OverrideImageQst)then exit(false);
        if Camera.GetPicture(PictureInstream, PictureDescription)then begin
            Clear(Rec."SSD Source Document");
            Rec."SSD Source Document".ImportStream(PictureInstream, PictureDescription, MimeTypeTok);
            Rec.Modify(true);
            exit(true);
        end;
        exit(false);
    ...
    begin
        DeleteExportEnabled:=Rec."SSD Source Document".Count <> 0;
    end;
