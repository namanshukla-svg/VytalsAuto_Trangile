Report 60140 "SSD_Sales Invoice"
{
    ...
    RDLCLayout = './Layouts/Sales Invoice new_1.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(Sales_Invoice_Header__ST38_No_; "Sales Invoice Header"."E-Way Bill No.")
            {
            }
            ...
            column(IRN_; "Sales Invoice Header"."IRN Hash")
            ...
            column(FooterAddress; StrSubstNo('%1 | %2 | %3 %4 | Phone %5', CompanyInfo."Registered Address", CompanyInfo."Registered Address 2", CompanyInfo."Registered City", CompanyInfo."Registered Post Code", CompanyInfo."Registered Phone No."))
            {
            }

            column(FooterFaxEmailHomepage; StrSubstNo('E-mail %1 | %2', CompanyInfo."Registered E-Mail", CompanyInfo."Registered Home Page"))
            {
            }
            dataitem(CopyLoop; "Integer")
            {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            */
                            ...
                            if SalesInvPackTemp.FindSet() then
                                repeat // "Sales Invoice Line".Reset();
                                    ...
                                    if PackDesc[PackCounter] = '' then
                                        PackDesc[PackCounter] := Format(SalesInvPackTemp."Package Count") + ' ' + Format(Item1."SSD Packing")
                                    else
                                        PackDesc[PackCounter] += CRLF + Format(SalesInvPackTemp."Package Count") + ' ' + Format(Item1."SSD Packing");
                                    if PackDescQty[PackCounter] = '' then
                                        PackDescQty[PackCounter] := Format(Abs(SalesInvPackTemp.Quantity)) + ' ' + Item1."Base Unit of Measure"
                                    else
                ...
                begin
                    ...
                    if TCSEntry.FindSet() then
                        repeat
                            TCS_Amount += TCSEntry."TCS Amount";
            ...
            begin
                ...
                if Location.Get("Location Code") then begin
                    ...
                    if not Location."SSD Automated Plant" then begin
                        ...
                        if StrLen("Sales Invoice Header"."Order No.") > 5 then
                            OrderNoNew := DelStr("Sales Invoice Header"."Order No.", 1, StrLen("Sales Invoice Header"."Order No.") - 5)
                        else
    ...
    begin
        ...
        if l_ValueEntry.Find('-') then
            repeat
                if l_ItemLedgerEntry.Get(l_ValueEntry."Item Ledger Entry No.") then begin
                    if p_SalesInvoiceLine2."Qty. per Unit of Measure" <> 0 then
                        Quantity := l_ValueEntry."Invoiced Quantity" / p_SalesInvoiceLine2."Qty. per Unit of Measure"
                    else
    ...
    begin
        ...
        if SalesInvoiceHeader.Find('-') then
            repeat
                l_SalesInvoiceLine2.SetRange("Document No.", SalesInvoiceHeader."No.");
                ...
                if l_SalesInvoiceLine2.Find('-') then
                    repeat
                        TotalQuantity := TotalQuantity + l_SalesInvoiceLine2.Quantity;
