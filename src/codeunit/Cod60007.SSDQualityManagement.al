codeunit 60007 "SSD Quality Management"
{
                ...
                if ReservationEntry.FindSet() then begin
                    ...
                    repeat
                        ...
                        if SSDQualityOrderHeader.FindFirst() then begin
                            ...
                            SSDQualityOrderHeader2."Zone Code" := WarehouseReceiptLine."Zone Code";
        ...
        if SSDQualityOrderHeader."Template Type" = SSDQualityOrderHeader."Template Type"::Manufacturing then begin
            ...
            if ItemLedgerEntry.FindSet() then
                repeat
                    ...
                    if SSDQualityOrderHeader."Decision For Quality Pass" = SSDQualityOrderHeader."Decision For Quality Pass"::Rejected then
                        SSDWMSSyncStatus.Status := SSDWMSSyncStatus.Status::Failed
                    else
                        SSDWMSSyncStatus.Status := SSDWMSSyncStatus.Status::Passed;
                    SSDWMSSyncStatus.Insert();
            ...
            if SSDQualitySamples.FindSet() then
                repeat
                    SSDWMSSyncStatus.Init();
                    SSDWMSSyncStatus."Entry No." := GetNextEntryNo();
                    SSDWMSSyncStatus."Location Code" := SSDQualityOrderHeader."Location Code";
    ...
    begin
        ...
        if ItemLedgerEntry.FindSet() then
            repeat
                ...
                SSDWMSSyncStatus.Quantity := ItemLedgerEntry.Quantity;
                SSDWMSSyncStatus."Qty. (Base)" := ItemLedgerEntry.Quantity;
                if SSDPostedQualityOrderHdr."Decision For Quality Pass" = SSDPostedQualityOrderHdr."Decision For Quality Pass"::Rejected then
                    SSDWMSSyncStatus.Status := SSDWMSSyncStatus.Status::Failed
                else
                    SSDWMSSyncStatus.Status := SSDWMSSyncStatus.Status::Passed;
                SSDWMSSyncStatus.Insert();
    ...
    procedure SyncQualityStatus(LocationCode: Code[10]; PackageId: Code[50]; QualityStatus: Enum "SSD Quality Status"): Boolean
    var
    ...
    procedure CreateQualityStatusJson(PackageId: Code[50]; QualityStatus: Enum "SSD Quality Status") JsonText: Text
    var
        JSonObject: JsonObject;
        WMSStatus: Text[30];
        DayInt: Integer;
    ...
    begin
        ...
        TransactionDate += '-';
        if StrLen(Format(MonthInt)) = 1 then
            TransactionDate += '0' + Format(MonthInt)
        else
            TransactionDate += Format(MonthInt);
        TransactionDate += '-';
        if StrLen(Format(DayInt)) = 1 then
            TransactionDate += '0' + Format(DayInt)
        else
    ...
    begin
        case QualityStatus of
            QualityStatus::Quarantine:
                WMSStatus := Format(QualityStatus);
    ...
    begin
        ...
        if SSDQualitySamples.FindLast() then
            NextLineNo := SSDQualitySamples."Line No." + 10000
        else
            NextLineNo := 10000;
    end;
    ...
    begin
        ...
        if SSDWMSSyncStatus.FindLast() then
            NextEntryNo := SSDWMSSyncStatus."Entry No." + 1
        else
            NextEntryNo := 1;
    end;
