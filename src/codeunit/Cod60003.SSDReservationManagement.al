codeunit 60003 "SSD Reservation Management"
{
    ...
    begin
        ...
        if TrackingSpecification."Item No." = '' then exit;
        if TrackingSpecification."Quantity (Base)" = 0 then
            TrackingSpecification."SSD Gross Weight" := 0
        else begin
            Item.Get(TrackingSpecification."Item No.");
    ...
    begin
        ...
        if WarehouseReceiptLine.FindSet() then
            repeat
                if SSDCommonFunctionAutomation.ItemPackageTracked(WarehouseReceiptLine."Item No.") then SyncWarehouseReceiptLine(WarehouseReceiptLine);
    ...
    begin
        ...
        if ProdOrderLine.FindSet() then
            repeat
                if SSDCommonFunctionAutomation.ItemPackageTracked(ProdOrderLine."Item No.") then SyncProductionOrderLine(ProdOrderLine);
    ...
    local procedure CreateItemLedgerEntryJson(ItemLedgerEntry: Record "Item Ledger Entry") JsonText: Text
    var
    ...
    begin
        ...
        // Atul::01122025  Item.TestField("SSD Storage Type");
        JsonText := '';
        ...
        JSonObject.Add('palletTypePreference', '');
        // JSonObject.Add('storageType', Format(Item."SSD Storage Type"));   // Atul::01122025
        JSonObject.Add('storageType', '');  // Atul::01122025
